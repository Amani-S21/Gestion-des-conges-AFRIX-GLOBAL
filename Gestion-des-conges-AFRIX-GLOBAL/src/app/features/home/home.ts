      ...
      <div class="relative rounded-[1.75rem] border border-(--color-surface) bg-(--color-surface) p-5 shadow-[0_8px_24px_rgba(61,124,140,0.1)] backdrop-blur-xl sm:p-7" aria-label="Aperçu du suivi des congés">
        ...
        <div class="relative">
          ...
          <div class="mt-3 space-y-3 rounded-2xl bg-(--color-bg)/80 p-4">
            ...
            <div class="flex items-center gap-2 pt-2 text-xs text-(--color-text-secondary)"><span class="size-2 rounded-full bg-(--color-success)"></span>Toutes les demandes sont synchronisées</div>
          </div>
        </div>
      </div>
    </section>
...
export class Home {
  availableDays = signal(25);
  pendingRequests = signal(2);
  approvalRate = signal(95);
}
