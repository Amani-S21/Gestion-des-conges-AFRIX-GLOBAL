      ...
      <nav class="w-full sm:block sm:w-auto" [class.hidden]="!menuOpen()" [class.block]="menuOpen()" aria-label="Navigation principale">
        <ul class="flex flex-col gap-2 sm:flex-row sm:items-center">
          ...
          @if (authService.isAuthenticated()) {
            ...
            <li>
              <a class="btn btn-secondary w-full no-underline sm:w-auto" routerLink="/conges" (click)="menuOpen.set(false)">Mes congés</a>
            </li>
            <li>
              <button class="btn btn-secondary w-full sm:w-auto text-red-300 hover:text-red-200" type="button" (click)="logout()">
                <app-icon name="close" />
                Déconnexion
              </button>
            </li>
            ...
            <li>
              <a class="btn btn-secondary w-full no-underline sm:w-auto" routerLink="/auth" (click)="menuOpen.set(false)">
